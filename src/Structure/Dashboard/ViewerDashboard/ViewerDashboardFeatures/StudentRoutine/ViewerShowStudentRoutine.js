...
const ViewerShowStudentRoutine = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_NODE_API}/api/routine/admin-search?school_info_id=${school_id}&teacher_id=${inputteacher}&class_id=${inputClass}&section_id=${inputsection}&day_id=${inputDay}&session_id=${inputsession}`,
        {
          headers: {
  ...
  }, [inputClass, school_id, inputsession, inputsection, inputDay, inputteacher])
  useEffect(() => {
    axios
  ...
  const handleSearch = () => {
    setInputClass(class_id)
    setInputDay(day)
    setInputSection(section_id)
    setInputSession(session_id)
    setInputTeacher(teacher_id)
